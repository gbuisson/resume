...
(def pdflatex-bin "pdflatex")

(defn run-pdflatex!
  "Compile a TeX file once with pdflatex from PATH, failing on any error."
  [file]
  (let [{:keys [exit out err] :as result}
        (shell/sh pdflatex-bin
                  "-halt-on-error"
                  "-interaction=nonstopmode"
                  file)]
    (when-not (zero? exit)
      (throw (ex-info (str "pdflatex failed for " file
                           (when-not (string/blank? err)
                             (str ":\n" err)))
                      {:command pdflatex-bin
                       :file file
                       :exit exit
                       :out out
                       :err err})))
...
(defn resume-data-as-pdf
  "Given a resume, render it as a PDF file."
  [file]
  (spit file
        (str
         (slurp "tex-header.tex")
         (resume-data-as-tex [:doc [:title] resume-data/resume])))
  ;; Two passes stabilize references such as the LastPage footer.
  (dotimes [_ 2]
    (run-pdflatex! file))
  file)
